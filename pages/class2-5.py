...
print([0, True, "a", 1.23]) # 這是一個有四個元素的list
...
print(L[::2])
# 就是取index 1到3的元素，不包含 index 4，所以是[2, 3, "a"]
print(L[1:4])
# 就是取index 1到3的元素，不包含 index 4，每次取2個元素，所以是[2, "a"]
print(L[1:4:2])
# 跟range一樣的用法，只是符號不同
...
# 這兩種方式都可以，但是看使用情境是否會需要index來決定要用哪一種方式
L = [1, 2, 3, "a", "b", "c"]
...
print("2號的平均成績:", (midterm[1] + final[1])/2)

name = ["Amy", "Mandy", "Peter"]
for i in range(len(name)):
    print(str((i)+1)+"號的名字是:"+name[i])