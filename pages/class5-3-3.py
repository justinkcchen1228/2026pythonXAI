...
openai.api_key = st.secrets["OPENAI_API_KEY"] # 設定OpenAI 的API金鑰
...
if "system_message" not in st.session_state: # 初始化系統訊息
    ...
    st.session_state.history.append({"role": "system", "content": st.session_state.system_message})

if "model" not in st.session_state: # 初始化AI模型
    st.session_state.model = "gpt-4o-mini" # 如果AI模型不存在，設置預設模型


# 設置三個列布局，分別占用 4:2:1 的寬度
