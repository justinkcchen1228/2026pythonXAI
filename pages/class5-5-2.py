...
openai.api_key = load_openai_api()  # 載入 OpenAI API 金鑰，API 金鑰是與 OpenAI 服務互動的必要憑證
...
# 尺寸選擇
size = st.selectbox(
    "圖像尺寸", ["1024x1024", "1536x1024", "1024x1536", "auto"], help="方形圖片生成速度最快，預設為 1024x1024 像素"
)

# 品質選擇
quality = st.selectbox("圖像品質", ["auto", "low", "medium", "high"], help="品質越高，生成時間越長，費用也越高")

# 輸出格式
output_format = st.selectbox("輸出格式", ["png", "jpeg", "webp"], help="選擇圖像輸出格式")

# 背景選項
background = st.selectbox("背景選項", ["auto", "transparent"], help="透明背景僅適用於 PNG 或 WebP 格式")
...
if output_format in ["jpeg", "webp"]:
    output_compression = st.slider("壓縮率", min_value=0, max_value=100, value=75, help="值越低檔案越小，但品質越差")

# 提示文字輸入區
prompt_text = st.text_area("提示詞", "A cute baby sea otter")

# 調節審核嚴格程度
moderation = st.selectbox("審核嚴格程度", ["auto", "low"], help="auto 為標準過濾，low 為較寬鬆的過濾")
